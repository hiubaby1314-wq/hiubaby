...
async function deleteFromR2(key) {
  if (!s3Client) return;
  let filename = key;
  if (key.startsWith('http')) filename = key.split('/').pop();
  await s3Client.client.send(new s3Client.DeleteObjectCommand({
    Bucket: R2_BUCKET, Key: `uploads/${filename}`
  }));
}
...
app.get('/api/materials', (req, res) => {
  ...
  res.json({ ok: true, items: items.map(item => ({ ...item, files: JSON.parse(item.files || '[]'), imgs: JSON.parse(item.imgs || '[]'), isNew: !!item.is_new, limit: !!item.limit_flag }))});
});
