...
// MiniMax T2A (Text-to-Audio) Proxy Endpoint
app.post('/api/minimax-tts', async (req, res) => {
  if (!MINIMAX_API_KEY || !MINIMAX_GROUP_ID) {
    return res.status(500).json({ error: 'MiniMax API credentials not configured on server.' });
  }

  const { text, voice_id, speed, emotion } = req.body;
  ...
  try {
    ...
    // MiniMax T2A v2 returns audio data directly as base64 in the 'data' field
    const data = await response.json();
    
    // 檢查錯誤響應
    if (data.base_resp && data.base_resp.status_code !== 0) {
       console.error('MiniMax Error Response:', data.base_resp);
